...
void read_traj(std::string file_name,int* theta, double* velocities, double* positions,
    ...
    if (file_buffer.open(file_name,std::ios::in))
    {
        ...
        theta=(int*) malloc(num_particles*max_meas_time*sizeof(int));
        velocities=(double*) malloc(3*num_particles*max_meas_time*sizeof(double));
        positions=(double*) malloc(3*max_meas_time*num_particles*sizeof(double));
        ...
        for (int step=0; step<num_timesteps;++step) {
            ...
            for (int atom=0; atom<num_atoms; ++atom) {
                ...
                if (line_split[2]==O_id) {

                    particle_idx=line_split[1]-1;
                    int my_2d_index=get_idx(step,particle_idx,num_particles,max_meas_time);
                    int my_3d_index=get_idx(step,particle_idx,0,num_particles,max_meas_time);
                    //cout << "my_3d_index: " << my_3d_index << endl;

                    for (int k=0; k<3; ++k) {
                        //int my_index=get_idx(step,particle_idx,k,num_particles,max_meas_time);
            ...
            for (int step_meas=0; step_meas < min(step,max_meas_time); ++step_meas) {
                ...
                for (int particle=0; particle < num_particles; ++particle) {
                    int my_2d_index_t0=get_idx(t0,particle,num_particles,max_meas_time);
                    int my_2d_index=get_idx(step,particle,num_particles,max_meas_time);
                    int my_3d_index_t0=get_idx(t0,particle,0,num_particles,max_meas_time);
                    int my_3d_index=get_idx(step,particle,0,num_particles,max_meas_time);
