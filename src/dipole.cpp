    ...
    if (file_buffer.open(file_name,std::ios::in))
    {
        ...
        for (int step=0; step<num_timesteps;++step) {
            ...
            for (int atom=0; atom<num_atoms; ++atom) {
                ...
                q=line_split[9];
                for (int k=0; k<3; ++k) {
                    //Mnet[3*step+k] += q*line_split[3+k]; // xyz components of dipole
                ...
                if (line_split[2]==O_id) {
                    ...
                    //        x0,x1,y0,y1,z0,z1) )
                    //{
                        //printf("I'm in the box at step %d, coordinates %f %f %f, my id: %d, my_2d_idx: %d\n",
                        //        step,positions[my_3d_index],positions[my_3d_index+1],positions[my_3d_index+2],
                        //        particle_idx,my_2d_index);
                        //theta[my_2d_index]=1;
                        N_occu_total+=1;
                        //for (int k=0; k<3; ++k) {
            ...
            //        if (theta[my_2d_index]==1) {
                for (int k=0; k<3; ++k) {
                    ...
                    //dipole_magnitude+=mu[my_3d_index+k]*mu[my_3d_index+k];
                }
                //dipole_magnitude=sqrt(dipole_magnitude);
                //cout << dipole_magnitude << endl;
            //        }
...
void read_dipole(std::string file_name, double* positions, double* Cm,
    ...
    if (file_buffer.open(file_name,std::ios::in))
    {
        ...
        ofstream CMout;
        CMout.open("results/C_M_out_w"+to_string(num_particles)+"_m"+to_string(max_meas_time)+"_t"+to_string(num_timesteps)+"-"+suffix);
        ofstream MNETout;
        MNETout.open("results/Mnet_out_w"+to_string(num_particles)+"_m"+to_string(max_meas_time)+"_t"+to_string(num_timesteps)+"-"+suffix);
        CMout << "#t Cm" << endl;
        for (int i=0; i< max_meas_time; ++i) {
            //RCout << plot_time[i] << " " << R[i] << endl;
            //CVout << plot_time[i] << " " << Cv[i] << endl;
